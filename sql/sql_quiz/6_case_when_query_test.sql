...
select (case when BALANCE_201312 > BALANCE_201311*1.1 then 1 else 0 end) as '성공률' from casa_201312;
select (sum(BALANCE_201312) - sum(BALANCE_201311)) as '증가분' from casa_201312;
select (sum(BALANCE_201312) - sum(BALANCE_201311))/ sum(BALANCE_201312) as '증가율' from casa_201312;
...
select student_id, (coalesce(math_score,0) + coalesce(eng_score,0)+ coalesce(phil_score,0)+ coalesce(music_score,0)) as total_sum from stud_score order by total_sum limit 1;
...
select student_id, stddev(math_score),stddev(eng_score),stddev(phil_score),stddev(music_score) from stud_score;
...
select sum(case when job = 'clerk' and eng_score >= 90 then current_sal*1.12 
when job = 'clerk' and eng_score < 90 then current_sal*1.07
when job = 'officer' and eng_score >= 90 then current_sal*1.1
when job= 'officer' and eng_score < 90 then current_sal*1.05
when job='manager' and eng_score >=90 then current_sal*1.08
when job ='manager' and eng_score <90 then current_sal*1.03
else current_sal end)- sum(current_sal) as '추가임금비용'  from staff_sal;
