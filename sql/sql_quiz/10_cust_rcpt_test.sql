...
*/
select cp.ssn, MOBILE_NO, count(ACCT_NO) as num_acct from cust_party cp left outer join rcpt_acct ra on cp.ssn = ra.ssn where cncl_Dt is null group by cp.ssn order by num_acct;

select tmp1.ssn, tmp1.party_nm, tmp1.mobile_no, tmp2.cnt, tmp2.rcpt_amt 
from cust_party tmp1 left outer join (select ssn, count(*) as cnt, sum(rcpt_amt) as rcpt_amt
from rcpt_acct where CNCL_DT is null group by ssn) tmp2 on tmp1.ssn = tmp2.ssn 
where cnt>=2 and rcpt_amt >= 500000 order by tmp1.ssn;
