...
2. 신규 무이자 할부 카드의 홍보대상 고객을 추출하시오. 일시불카드를 사용한적이 있는 사람
...
select cmf, party_nm, cash_amt from card_tran_201311 order by CASH_AMT desc;

select cmf, party_nm, (coalesce(pif_amt, 0) + coalesce(inst_amt, 0)+ coalesce(ovrs_amt, 0) + coalesce(cash_amt,0)) as total_amt from card_tan_201311;
