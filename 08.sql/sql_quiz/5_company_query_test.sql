...
INSERT ignore INTO CUST_INFO VALUES ('6705302777666', 'MIJA',0 ,'HAN', -76.77);
...
select substring(name,2,4) as name_str from vendor_info;
...
select staff_nm, avg(round(datediff(now(),birth_dt)/365)) as age from clerk group by gender;
select def_nm, avg(round(datediff(now(),birth_dt)/365)) as age from clerk group by def_nm;
